...
<a href='../Week/Graph.php'><button style='margin-left:20%;width:100px;height:30px;font-size:100%;'>週期模式</button></a>
...
<!------------------------------------日期選擇--------------------------->
	...
	<div class="div1">
	<p style="font-size:30px;">請選擇月份:</p>
	</div>
	<div class="div1">
	<input type="month" name="Month" style='width:150px;height:30px;font-size:20px;' value="<?php if(!empty($_POST['Month'])){echo $_POST['Month'];}else if(!empty($Final_Day)){$timestamp_1=strtotime($Final_Day);echo date('Y-m',$timestamp_1);}else{date('Y-m');}?>" min="<?php if(!empty($Inital_Day)){$timestamp_0=strtotime($Inital_Day);echo date('Y-m',$timestamp_0);}else{date('Y-m');}?>" max="<?php if(!empty($Final_Day)){$timestamp_1=strtotime($Final_Day);echo date('Y-m',$timestamp_1);}else{date('Y-m');}?>" size="50">
	</div>
