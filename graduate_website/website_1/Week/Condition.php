	...
	if(isset($_POST['ID']))
	{
		...
		if(!empty($_POST['Month']) and !empty($_POST['Class']))
		{
			...
			if($Class=='All')
			{
				...
				$sql="select * from conditions where RaspberryID='$RaspberryID' and WEEK(Day)=WEEK('$Month') and YEAR(Day)=YEAR('$Month') ORDER BY Day";
			}
			else
			{
				
				$sql="select * from conditions where RaspberryID='$RaspberryID' and WEEK(Day)=WEEK('$Month') and YEAR(Day)=YEAR('$Month') and $Class>0 ORDER BY Day";
			}
